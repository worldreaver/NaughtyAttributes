...
namespace Worldreaver.RectEx
{
    internal static class MiscellaniousExtensions
    {
        ...
        {
            return new Rect(x: rect.y, y: rect.x, width: rect.height, height: rect.width);
        }

        public static Rect Union(this Rect rect,
            params Rect[] other)
        {
            ...
            else
            {
                ...
                return Rect.MinMaxRect(xmin: xMin, xmax: xMax, ymin: yMin, ymax: yMax);
            }
        }

        public static Rect Intend(this Rect rect,
            float border)
        {
            rect = rect.Abs();

            var result = new Rect(x: rect.x + border, y: rect.y + border, width: rect.width - 2 * border, height: rect.height - 2 * border);
        ...
        {
            rect = rect.Abs();
            return new Rect(x: rect.x - border, y: rect.y - border, width: rect.width + 2 * border, height: rect.height + 2 * border);
        }
